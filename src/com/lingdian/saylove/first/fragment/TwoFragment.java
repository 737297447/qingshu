...
public class TwoFragment extends BaseFragment {
	...
	public void initView() {
		...
		Cursor cursor = db.rawQuery("select * from 	Xiangshi where single='Gril'",
				null);
		sqlTime = new String[cursor.getCount()];
		sqlGanshou = new String[cursor.getCount()];
		int i = 0;
		if (!cursor.moveToNext()) {
			first_time.setText("相识在夏天");
			dazi = "你不再是一个观众，从我认识你的那一天开始；你不再无关紧要，当我爱上你之后，我只想对你说，我绝不放手！";
		}else{
		...
		if (jpgFiles.length == 0) {
			iWorkImageView.setImageResource(R.drawable.first_02);
		} else {
